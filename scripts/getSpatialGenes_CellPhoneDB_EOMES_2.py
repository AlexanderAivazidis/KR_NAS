...
# Rank genes by correlation with spatially varying EOMES+ genes:

expressionData1 = np.array(resid_expr_subset.loc[:,variableLigands])
expressionData1 = expressionData1[np.argsort(metadata['VCDepth'][subset_rois]),:]
expressionData1 = expressionData1[:20]
...
correlationMatrix = np.corrcoef(expressionDataDF)

sns.clustermap(expressionDataDF.T)

plt.imshow(expressionDataDF)
