...
vRG_markers = markers[markers[,'cluster'] == 'oRG' & markers[,'p_val_adj'] == 0,'X'][1:50]
IP_markers = markers[markers[,'cluster'] == 'IP' & markers[,'p_val_adj'] == 0,'X']
...
counts = counts[,metadata$Sanger_sampleID]
...
counts_subset = counts_subset[rowCounts > 3000, ]
genes = genes[rowCounts > 3000]
...
pdf('BeautifulVolcanoVeryTall.pdf', width = 10, height = 10)
EnhancedVolcano(res,
                lab = genes,
                x = 'log2FoldChange',
                y = 'pvalue',
