...
class Pymc3Model(BaseModel):
    ...
    def fit_advi_iterative(self, n=3, method='advi', n_type='restart', n_iter=None, progressbar=True):
    ...
    def fit_advi_refine(self, n_iter=10000, progressbar=True):
    ...
    def b_evaluate_stability(self, node, n_samples=1000):
    ...
    def sample_posterior(self, node='all', n_samples=1000,
                         save_samples=False, return_samples=False,
        ...
        :param return_samples: return samples in addition to saving them in `self.samples`
        ...
        for i in tqdm(range(int(self.tracking_n_steps+1))):
            ...
            for n_init in init_names:
                ...
                if df_prior_node_name1 is not None:
                    ...
                    factors_df.columns = factors_df.columns + '|' + pd.Series(np.char.mod('%d', np.around(df_prior_1.flatten(), 1)))
                if df_prior_node_name2 is not None:
                    df_prior_2 = self.samples['post_sample_means'][df_prior_node_name2]
                    factors_df.columns = factors_df.columns + '|' + pd.Series(np.char.mod('%d', np.around(df_prior_2.flatten(), 1)))
    ...
    def sample_posterior_bootstrap(self, node='all', n_samples=100, save_samples=True):
        ...
        self.samples['post_sample_q05'] = {v: np.quantile(post_samples[v], 0.05, axis=0) for v in post_samples.keys()}
        self.samples['post_sample_q95'] = {v: np.quantile(post_samples[v], 0.95, axis=0) for v in post_samples.keys()}
