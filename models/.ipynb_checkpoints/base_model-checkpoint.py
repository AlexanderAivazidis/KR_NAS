...
class BaseModel():
    ...
    def align_plot_stability(self, fac1, fac2, name1, name2):
        r""" Align columns between two np.ndarrays using scipy.optimize.linear_sum_assignment,
        then plot correlations between columns in fac1 and fac2, ordering fac2 acrording to alignment
        ...
        fig, ax = plt.subplots()
        im = ax.imshow(corr12[:, linear_sum_assignment(2 - corr12)[1]])
        plt.title('Training initialisation ' + name1 + ' vs ' + name2)

        plt.tight_layout()
        
    def generate_cv_data(self, n=2):
        ...
        self.X_data_sample[1] = self.X_data - self.X_data_sample[0]
        
    def bootstrap_data(self, n=10, downsampling_p=0.8):
        r""" Generate X_data for bootstrap analysis by sampling molecule counts 
             with np.random.binomial
        :param n: number of bootstrap samples to generate
        :param downsampling_p: sample this propotion of values
        """
    ...
    def print_gene_loadings(self, gene_fact_name = 'gene_factors',
        ...
        gene_loadings = self.gene_loadings.copy()
    ...
    def plot_loading_distribution(self, loadings_name = 'gene_factors',
        ...
        :param loadinds: np.ndarray to be plotted column-wise. Supersedes `loadings_name`.
        """
    ...
    def export2adata(self, adata, slot_name='mod'):
        r""" Add posterior mean and sd for all parameters to unstructured data `adata_sel.uns['mod']`.
        ...
        adata.uns[slot_name]['fact_names'] = self.fact_names
