...
class WTACounts_GeneralModel(Pymc3Model):
    ...
    def __init__(
        ...
        with self.model:
            ...
            self.celltype_expression =  pm.Deterministic('celltype_expression', pm.math.dot(self.w_celltype, self.h.T))
            self.background_expression =  pm.Deterministic('background_expression', pm.math.dot(self.w_background, self.h.T))
            
            # Expected gene counts are sum of gene expression and background counts, scaled by library size:
            self.expression_ring = self.ring_proportion_celltype * self.celltype_expression + 
                                   self.ring_proportion_background * self.background_expression
            self.expression_aoi = self.aoi_proportion_celltype * self.celltype_expression + 
                                   self.aoi_proportion_background * self.background_expression
