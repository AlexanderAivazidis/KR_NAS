...
r"""The Co-Location model decomposes the expression of genes across locations into a set
    ...
    Unlike the standard CoLocationModel recommended for Visium data, this model includes a
    non-specific binding term that scales linearly with the total number of counts in the region
    of interest. In addition, it expects negative probe counts in each region of interest to estimate this
    background. Finally, this version uses a Poisson instead of a Negative Binomial to model 
    residual noise. The Poisson is approximated as a Gamma distribution for counts greater than 1000.
    At this stage we find this is most robut for NanostringWTA data."""
...
class ProbeCounts_CoLocationModel_NB_V2(Pymc3LocModel):
    ...
    * **combs_per_spot** prior tells the model how much co-location signal to expect between the programmes / cell types.
    ...
    def plot_Locations_1D_scatterPlot(self, x, order = None, polynomial_order = 6, figure_size = (30,30),
            ...
            for i in range(len(self.fact_names)):
                ...
                if density:
                    results = (results.T/[sum(results[i,]) for i in range(len(results[:,1]))]).T
                y = results[:,self.fact_names == self.fact_names[i]][:,0]
                plt.plot(np.unique(x[order]), np.poly1d(np.polyfit(x[order], y, polynomial_order))(np.unique(x[order])), c = cellColours[0])
    ...
    def plot_Locations_1D_scatterPlot_multipleCategories(self, x, order = None, polynomial_order = 6, figure_size = (30,30),
    ...
    def plot_Locations_1D_scatterPlot_secondModel(self, mod2, x, order = None, polynomial_order = 6, figure_size = (30,30),
                                  saveFig = None, density = True, xlabel = 'x-coordinate', categories = [None]):
