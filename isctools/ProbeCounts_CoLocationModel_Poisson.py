...
class ProbeCounts_CoLocationModel_Poisson(Pymc3LocModel):
    ...
    * **combs_per_spot** prior tells the model how much co-location signal to expect between the programmes / cell types.
    ...
    def __init__(
        ...
        # Total number of genes in each region of interest, divided by 10^5
    ...
    def logp_Poisson(self, value):
        ...
        return tt.switch(tt.gt(self.mu_biol,1e3),
                         pm.Gamma.dist(mu = self.mu_biol, sd = tt.sqrt(self.mu_biol)).logp(value),
                         tt.switch(tt.eq(self.mu_biol, 0) * tt.eq(value, 0),
                         0, bound(logpow(self.mu_biol, value) - factln(value) - self.mu_biol, self.mu_biol >= 0, value >= 0)))
